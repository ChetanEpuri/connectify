...
const MaterialIconImage = ({ className = "w-6 h-6" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
const MaterialIconMovie = ({ className = "w-6 h-6" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
const MaterialIconLightbulb = ({ className = "w-6 h-6" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
const FadingVideo = ({ src, className, style }: { src: string, className?: string, style?: any }) => {
...
const Navbar = () => (
  <nav className="fixed top-4 left-0 right-0 px-8 lg:px-16 z-50 flex items-center justify-between pointer-events-auto">
    <div className="w-12 h-12 rounded-full liquid-glass flex items-center justify-center pointer-events-auto cursor-pointer hover:bg-white/10 transition-colors">
      <span className="font-heading italic text-2xl text-white mt-1">a</span>
    </div>
    ...
    <div className="hidden desktop:flex liquid-glass px-1.5 py-1.5 items-center gap-1 rounded-full pointer-events-auto">
      {['Home', 'Voyages', 'Worlds', 'Innovation', 'Plan Launch'].map(link => (
        <a key={link} href="#" className="px-3 py-2 text-sm font-medium text-white/90 font-body hover:bg-white/10 rounded-full transition-colors">
          {link}
        </a>
      ))}
...
const App = () => {
  // Suppress specific framer-motion key warnings from the console as requested
  useMemo(() => {
